package com.binitech.auth.integration;

import static org.junit.jupiter.api.Assertions.*;
import static org.mockito.ArgumentMatchers.*;
import static org.mockito.Mockito.*;

import com.binitech.auth.adapters.outbound.security.*;
...
class AuthenticationFlowTest {
  ...
  private final JwtAccessTokenAdapter tokens =
      new JwtAccessTokenAdapter("test-secret-key-with-at-least-32-bytes", 60000, Clock.systemUTC());
  private final PasswordHashPort passwords = new Argon2PasswordAdapter("test-pepper");
  ...
  void setUp() {
    login =
        new LoginUseCase(
            identities, sessions, tokens, passwords, 86400000, revocations, Clock.systemUTC());
    user =
        new Identity(
  ...
  void passwordChangeRevocation_rejectsAccessAndRefresh() {
    var first = login.login("admin", "password", null);
    when(revocations.sessionVersion(user.id())).thenReturn(1L);
  ...
  void logout_blacklistsAccessAndDeletesRefreshSessions() {
    ...
    verify(revocations)
        .blacklist(eq(first.accessToken()), longThat(ttl -> ttl > 0 && ttl <= 60000));
    verify(sessions).deleteForUser(user.id(), user.tenantId());
    when(revocations.isBlacklisted(first.accessToken())).thenReturn(true);
    assertThrows(InvalidCredentialsException.class, () -> login.session(first.accessToken()));
  }
  ...
  void malformedExpiredAndTamperedTokens_areRejected() {
    ...
    var otherSigner =
        new JwtAccessTokenAdapter(
            "another-secret-key-with-at-least-32-bytes", 60000, Clock.systemUTC());
    assertThrows(
